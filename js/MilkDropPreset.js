...
define(function (require) {
    ...
    var MilkdropPreset = Class({
        constructor: function (glu, name, preset, gx, gy) {
            this.name = name;
            this.inputs = {};
            this.outputs = {
                dx: 0,
            ...
            this.initialVals = new Variables.PresetFrameVariablePool();
            for (var prop in preset)
                this.loadParam(prop, preset[prop]);

            this.framePool = new Variables.PresetFrameVariablePool();
            this.pixelPool = new Variables.PresetPixelVariablePool();
            ...
            for (var i = 0; i < this.customShapes.length; i++)
                this.customShapes[i] = new Renderable.Shape(glu, this.customShapes[i], this.initialQs);
            for (var i = 0; i < this.customWaves.length; i++)
                this.customWaves[i] = new Renderable.CustomWaveform(glu, this.customWaves[i], this.initialQs);


            this.outputs.darkenCenter = new Renderable.DarkenCenter(glu, this.outputs.darkenCenter);
            this.outputs.mv = new Renderable.MotionVectors(glu, this.outputs.mv);
            this.outputs.border = new Renderable.Border(glu, this.outputs.border);
            this.outputs.wave = new Renderable.MilkdropWaveform(glu, this.outputs.wave);
            this.outputs.videoEcho = new Renderable.RenderItem(glu, this.outputs.videoEcho);

            this.outputs.brighten = new Renderable.Brighten(glu, this.outputs.brighten);
            this.outputs.darken = new Renderable.Darken(glu, this.outputs.darken);
            this.outputs.invert = new Renderable.Invert(glu, this.outputs.invert);
            this.outputs.solarize = new Renderable.Solarize(glu, this.outputs.solarize);
        ...
        varInit: function () {
            var testPool = new Variables.PresetFrameVariablePool();
            var testPixPool = new Variables.PresetPixelVariablePool();
            var winProps = {};
